...
export function UserMenu({ onNavigate }: { onNavigate: (path: string) => void }) {
  ...
  const name = userDisplayName(profile) || (me.isLoading ? `${t("common.loading")}...` : t("common.unknown"));
  ...
  return (
    <DropdownMenu>
      <DropdownMenuTrigger className="user-menu-trigger" aria-label={t("nav.userMenu")}>
